...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const emailPromises = (nearbyUsers as Profile[]).map(user => {
        // Constrói um link direto para o mapa, centrado na localização do alerta.
        // Link para o mapa, centrado na localização do alerta.
        const mapUrl = `https://rubensousa100.github.io/Aldeias-Seguras-Site/Incendio.html?lat=${newRelato.lat}&lng=${newRelato.lng}&zoom=13#ffr-map-collab`;
        ...
        return fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: 'Aldeias Seguras <onboarding@resend.dev>', // Endereço de teste do Resend. Funciona sem domínio próprio.
            to: [user.email],
